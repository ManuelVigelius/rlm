...
1. A `context` variable that contains extremely important information about your query. You should check the content of the `context` variable to understand what you are working with. Make sure you look through it sufficiently as you answer your query.
2. A `llm_query` function that allows you to query an LLM (that can handle around 500K chars) inside your REPL environment.
3. A `get_features` function that allows you to extract features and statistics from the context
...
```repl
chunk = context[:1000]
answer = llm_query(f"What is the min and max value of this chunk? Here is the chunk: {{chunk}}")
print(answer)
```
...
This is a special case and your context will be a pandas DataFrame that represents a time series.
You can still do arbitrary transformations on it, but additionally you can query statistics of intervalls:
```repl
print('Statistics regarding the full time series')
get_features(context)

print()
print('Statistics in the period of 2020')
get_features(context, start='01.01.2020', end='2020-01-02 12:00')
```
